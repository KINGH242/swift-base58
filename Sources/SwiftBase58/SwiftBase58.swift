...
public enum Base58 {
    ...
    private static var zero: BigUInt { 0 }
    private static var radix: BigUInt { BigUInt(alphabet.count) }
    
    /// Encode the given bytes into a Base58Check encoded string.
    /// - Parameter bytes: The bytes to encode.
    /// - Returns: A base58check encoded string representing the given bytes.
    ...
    /// - Parameter bytes: The bytes to encode.
    /// - Returns: A base58 encoded string representing the given bytes.
